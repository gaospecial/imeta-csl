...
url <- paste0("https://api.zotero.org/users/", user_id, "/items/top?format=json")
...
```{r}
item_data_language_2 <- item_data_title  |> 
...
```{r}
items  |> pull(language) |> table() |> sort(decreasing = TRUE) |> barplot()
```
...
for (i in 1:n) {
  item_batch = data[(i-1)*50 + 1:min(i*50, nrow(data)), ] |> 
    mutate(language = "en") |> 
...
for (item in items) {
  ...
  # 更新文献
  response <- PUT(url, add_headers("Zotero-API-Key" = key), body = toJSON(item), encode = "json")
}
